...
public partial class Z80Emulator
{
    ...
    /// Will be slower than executing steps normally due to resetting the state at the end and delegate overhead from calling <paramref name="performActionRequired"/>.
    /// </remarks>
    /// <param name="performActionRequired">Perform any action required at the end of a step.</param>
    public void ExecuteInstruction(Action<ActionRequired> performActionRequired)
    {
        ...
        while (true)
        {
            ...
            switch (currentStep)
            {
                // If we're on step 0, we must have completed the instruction or be reading the second byte of an opcode.
                case OpcodeReadStep0:
                case HaltedStep0:
                    // If we're not reading the opcode, then we must have completed the instruction. Perform any final action and finish.
                    // No need to check the opcode table as reading the second byte of an opcode is done with an overlapped read.
                    if (!readingOpcode)
                    {
